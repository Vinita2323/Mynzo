...
export default function BrandPage() {
  ...
  if (loading) {
    return (
      <div className="flex-grow space-y-6 pb-12 bg-slate-50 animate-pulse">
        {/* Header Skeleton */}
        <div className="bg-white border-b border-slate-150 py-8">
        ...
        <div className="max-w-7xl mx-auto px-4 pt-6 space-y-4">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2">
            {[1, 2, 3].map(i => (
              <div key={i} className="w-40 aspect-[3/4.2] bg-slate-200 rounded-2xl flex-shrink-0" />
            ))}
          </div>
        </div>
      </div>
  ...
  if (!brand) {
    return (
      <div className="flex-grow flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-50">
        ...
        <button
          onClick={() => navigate('/')}
          className="mt-6 px-5 py-2 bg-[#ee4923] text-white rounded-xl text-[10.5px] font-black uppercase tracking-wider hover:scale-105 active:scale-95 transition-all shadow-md"
        >
          Go Back Home
        </button>
      </div>
  ...
  return (
    <div className="flex-grow space-y-8 pb-16 bg-slate-50/50">
      
      {/* Brand Header Section */}
      <div className="bg-white border-b border-slate-200/80 py-8 relative">
        <div className="max-w-7xl mx-auto px-4">
          {/* Back Button and Info Row */}
          <div className="flex items-center gap-4 mb-6">
            <button
              onClick={() => navigate(-1)}
              className="w-9 h-9 bg-slate-100 text-slate-700 rounded-full flex items-center justify-center hover:bg-slate-200 hover:scale-105 active:scale-95 transition-all cursor-pointer"
            >
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-5 md:gap-6">
            {/* Brand Logo */}
            <div className="w-20 h-20 md:w-28 md:h-28 rounded-2xl bg-white border border-slate-200 p-2 flex items-center justify-center shadow-xs overflow-hidden flex-shrink-0 animate-scale-in">
              <img 
        ...
        {popularProducts.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[9.5px] bg-orange-50 border border-orange-100 text-[#ee4923] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                Best Sellers
              </span>
            </div>
